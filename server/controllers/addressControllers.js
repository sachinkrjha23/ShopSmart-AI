...
export const addAddress = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!/^\d{6}$/.test(pincode)) {
    return next(
      new ErrorHandler("Please provide a valid 6-digit pincode.", 400),
    );
  }

  // 4. Check for exact duplicate address
  const duplicate = await database.query(
    `SELECT id FROM user_addresses 
     ...
     AND pincode = $5
     AND phone = $6`,
    [req.user.id, address, city, state, pincode.trim(), phone.trim()],
  );
  ...
  // 5. Check max 5 addresses per user
  ...
  if (parseInt(count.rows[0].count) >= 5) {
    return next(
      new ErrorHandler(
        "Maximum 5 addresses allowed. Please delete one to add a new address.",
        400,
      ),
    );
  }
  ...
  try {
    ...
    // 6. If this is default, unset all others first
    if (is_default) {
      await client.query(
    ...
    // 7. If this is the first address, make it default automatically
...
export const editAddress = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    // 4. Check for exact duplicate against ANOTHER address
    const resolvedAddress = address?.trim() || existing.rows[0].address;
    const resolvedCity = city?.trim() || existing.rows[0].city;
    const resolvedState = state?.trim() || existing.rows[0].state;
    const resolvedPincode = pincode?.trim() || existing.rows[0].pincode;
    const resolvedPhone = phone?.trim() || existing.rows[0].phone;
    ...
    if (duplicate.rows.length > 0) {
      return next(
        new ErrorHandler(
          "Another address with these exact details already exists.",
          400,
        ),
      );
    }
