...
import { fetchPublicSellerProfile, clearPublicSellerProfile } from '../store/slices/sellerSlice'
...
const SellerProfile = () => {
  ...
  const { publicSellerProfile: seller, loading: sellerLoading, error } = useSelector((state) => state.seller)
  const { products, pagination, loading: productsLoading } = useSelector((state) => state.product)
  ...
  if (!seller) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-gray-500 mb-6">{error || "This seller doesn't exist or isn't currently active."}</p>
        <Link to="/products">
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col gap-8">
      <div className="bg-white rounded-xl border border-gray-100 p-6 flex items-start gap-4">
        ...
        <div>
          ...
          {seller.description && (
            <p className="text-sm text-gray-600 mt-3 max-w-2xl">{seller.description}</p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <ProductGrid products={products} loading={productsLoading} columns={4} />
