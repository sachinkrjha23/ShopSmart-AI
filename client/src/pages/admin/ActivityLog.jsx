...
const ACTION_LABELS = {
  ...
  item_fulfillment_updated: { label: 'Item Fulfillment Updated', variant: 'info' },
...
const ENTITY_TYPES = ['seller', 'return', 'order', 'order_item', 'review', 'contact_message', 'user', 'product', 'report']

const ActivityLog = () => {
  const dispatch = useDispatch()
  const { activityLogs, totalActivityLogs, activityLogPage, loading } = useSelector((state) => state.admin)
  const [filters, setFilters] = useState({ actionType: '', entityType: '', fromDate: '', toDate: '' })
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {activityLogs.map((log) => {
                const meta = ACTION_LABELS[log.action_type] || { label: log.action_type, variant: 'default' }
                ...
                return (
                  <>
                    <tr key={log.id} className="border-t border-gray-100 align-top">
                      ...
                      <td className="px-4 py-3 text-gray-600">
                        {log.admin_name ? (
                          <>
                            {log.admin_name}
                            <span className="block text-xs text-gray-400">{log.admin_email}</span>
                          </>
                        ) : (
                          <span className="text-gray-400 italic">Deleted admin</span>
                        )}
                      </td>
                    ...
                    {isOpen && (
                      <tr key={`${log.id}-details`} className="border-t border-gray-50 bg-gray-50">
