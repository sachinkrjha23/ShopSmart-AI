...
const AdminOrderDetail = () => {
  ...
  useEffect(() => {
    ...
    return () => dispatch(clearAdminSingleOrder());
  }, [dispatch, id]);
  ...
  const handleAdvanceStatus = async () => {
    ...
    try {
      await dispatch(
        updateOrderStatus({ orderId: id, status: nextStatus }),
      ).unwrap();
  ...
  const handleCancelConfirm = async () => {
    ...
    try {
      const result = await dispatch(cancelAdminOrder(id)).unwrap();
      toast.success(
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6">
        ...
        {nextStatus || canCancel ? (
          ...
          <p className="text-sm text-gray-500">
            This order is {order.order_status.toLowerCase()} and cannot be
            changed further.
          </p>
        )}
      </div>
      ...
      <ConfirmDialog
        ...
        title="Cancel Order"
        message="Are you sure you want to cancel this order? This will restock items and initiate a refund if the order was paid. This cannot be undone."
