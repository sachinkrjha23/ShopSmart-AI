...
import { fetchSingleOrder, cancelOrder, clearSingleOrder } from '../store/slices/orderSlice'
...
const PAYMENT_TAGS = {
  Pending: { label: 'Payment Pending', className: 'bg-yellow-50 text-yellow-700' },
...
const OrderDetail = () => {
  ...
  const { singleOrder: order, loading, error } = useSelector((state) => state.order)
  ...
  const subtotal = Number(order.total_price) + Number(order.discount_amount || 0) - Number(order.shipping_price)
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 mb-6">
        <div className="flex items-start justify-between flex-wrap gap-3 mb-6">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            {paymentTag && (
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${paymentTag.className}`}>
                {paymentTag.label}
              </span>
            )}
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6">
        ...
        <div className="flex flex-col gap-2 text-sm">
          ...
          <div className="flex justify-between text-gray-600">
            <span>Shipping</span>
            <span>{Number(order.shipping_price) > 0 ? `₹${order.shipping_price}` : 'Free'}</span>
          </div>
        ...
        {canCancel && (
          <div className="mt-6">
            <Button variant="danger" onClick={() => setCancelDialogOpen(true)} disabled={loading}>
              Cancel Order
            </Button>
          </div>
        )}
