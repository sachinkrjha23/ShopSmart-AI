...
const NotificationBell = () => {
  ...
  const { bellNotifications: notifications, unreadCount: count } = useSelector(
  ...
  const handleNotificationClick = (notification) => {
    ...
    if (
      notification.link_entity_type === "order" &&
      notification.link_entity_id
    ) {
      navigate(`/orders/${notification.link_entity_id}`);
    } else if (
      notification.link_entity_type === "seller" &&
      notification.link_entity_id
    ) {
  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      {isOpen && (
        <div className="fixed inset-x-4 top-16 sm:absolute sm:inset-x-auto sm:top-auto sm:right-0 sm:mt-3 sm:w-80 bg-white rounded-xl shadow-lg border border-gray-100 z-50 overflow-hidden">
